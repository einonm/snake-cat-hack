...
train_X, val_X, train_y, val_y = train_test_split(allcat.drop('age_years', axis=1), 
                                                  allcat.age_years)
...
dummy = dtr_model.fit(train_X, train_y)

val_predictions = dtr_model.predict(val_X)

dummy = plt.hist(val_predictions)

dummy = plt.hist(val_y)
